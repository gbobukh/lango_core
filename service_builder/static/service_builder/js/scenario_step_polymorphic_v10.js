(function () {
    ...
    function init($) {
        if (window.__scenarioStepPolymorphicV10Initialized) return;
        window.__scenarioStepPolymorphicV10Initialized = true;
        ...
        $(document).on('change', 'select[id$="-step_type"]', function () {
            initRow($(this).closest('.dynamic-steps'));
        });

        $(document).on('change', 'select[id$="-action_type"]', function () {
            populateConfigTemplate($(this).closest('.dynamic-steps'));
        });

        $('#id_arguments').on('change keyup', function () {
            setTimeout(function () {
                $('.dynamic-steps').each(function () { convertAuthInputToSelect($(this)); });
            }, 300);
        });

        $('.dynamic-steps').each(function () { initRow($(this)); });
        $(document).on('formset:added', function (_event, row) { initRow($(row)); });
    }
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        else console.error('scenario_step_polymorphic_v10: jQuery not available.');
    });
})();
