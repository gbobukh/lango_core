...
class ScenarioStepForm(ClickToEditFormMixin, forms.ModelForm):
    ...
    def clean(self):
        ...
        if step_type == 'API_CALL' and not method:
            self.add_error('method', "Service Method is required for API Call steps.")
            
        if step_type == 'ACTION' and not action_type:
            self.add_error('action_type', "Action Type is required for Action steps.")
